...
class SPxApp:
    ...
    def _build_ui(self):
        ...
        ttk.Entry(endmembers_frame, textvariable=self.endmembers_var).pack(
        ...
        ttk.Button(edit_frame, text="Edit Thresholds", command=self._open_thresholds_dialog).pack(
            side=tk.LEFT, padx=(0, 6)
        )
        ttk.Button(edit_frame, text="Edit Axis Labels", command=self._open_axis_labels_dialog).pack(
            side=tk.LEFT, padx=(0, 6)
        )
        ...
        self.open_btn = ttk.Button(
            status_frame, text="Open Output Folder", command=self._open_output, state=tk.DISABLED
        )
